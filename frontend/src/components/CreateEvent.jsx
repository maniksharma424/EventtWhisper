...
const CreateEvent = ({ day, month, year,hideEventSchdeuler }) => {
  ...
  return (
    ...
    Create Event
    <button className="font-[300] px-2 border-[1px] text-[15px] rounded-md shadow-sm" onClick={()=>{
      dispatch(removeBlur())
    ...
    >x</button>
      ...
      <div id="date-time" className="h-fit w-full flex flex-col justify-between items-start sm:text-[15px] text-[10px] ">
        ...
        <div className="sm:w-2/5 w-4/5 h-full  ">
          ...
          <span className="w-full flex justify-around sm:shadow-md items-center p-1 bg-gray-100 sm:rounded-lg">
            ...
            <select className="form-select w-1/4 sm:rounded-md  ">
        ...
        <p className="text-[10px] tracking-wider mt-1 text-gray-500">This event will take place on {`${day} ${monthNames[month]} ,${year}`} at 2.00PM </p>
      </div>
      <div className="create-cancel-btns  w-full h-fit">
        <button className="border-[1px] w-full border-gray-500 px-4 py-1 rounded-2xl"
        onClick={()=>{
          dispatch(removeBlur())
          dispatch(defaultOpacity())
          hideEventSchdeuler()
